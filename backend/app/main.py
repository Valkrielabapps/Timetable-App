...
# TEMPORARY - delete once Sentry has been confirmed working in production.
# Deliberately raises so the whole reporting path (exception -> scrubbing ->
# Sentry) can be exercised on the real deployment, which is the only place
# the DSN, network egress and environment tag are actually in play.
# Authentication is required so this cannot be used to flood the error quota.
@app.get("/api/debug/boom", include_in_schema=False)
def _sentry_smoke_test(current_user: User = Depends(get_current_user)):
